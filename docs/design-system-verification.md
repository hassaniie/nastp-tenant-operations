...
The subsequent shell review moved Admin navigation into the canonical `patterns/admin-sidebar.tsx`, added independent group expansion and distinct module/page states, and replaced the static topbar label with the canonical functional Breadcrumb. Browser checks confirmed group expansion, full rail collapse/restore, linked Workspace and Energy ancestors, a current Charges & Billing leaf, light/dark parity, and zero horizontal overflow. Evidence: `sidebar-breadcrumb-dark.jpg`, `sidebar-pattern-dark.jpg` and `sidebar-pattern-light.jpg`.
