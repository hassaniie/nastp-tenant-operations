    ...
    <Page>
        ...
        <Card>
          ...
          <CardBody className="grid content-start grid-cols-2 gap-3 sm:grid-cols-4">
            {(['energy', 'genset', 'peak', 'off_peak'] as ChargeComponent[]).map((c) => (
              <div key={c} className="rounded-xl border border-border bg-muted/50 p-3 text-center">
            ...
            <div className="col-span-full flex items-start gap-2 rounded-xl border border-border bg-muted/40 p-3 text-[12px] text-muted-foreground">
          ...
          <CardBody className="flex flex-col gap-4">
            ...
            <div className="rounded-xl border border-border bg-card p-3.5">
              <div className="flex items-center justify-between text-[12px]">
                <span className="text-muted-foreground/75">Current monthly (blended)</span>
                <span className="tnum font-medium text-foreground">{currency(impact.currentBill, { compact: true })}</span>
              </div>
              <div className="mt-1.5 flex items-center justify-between text-[12px]">
                <span className="text-muted-foreground/75">Proposed monthly</span>
              ...
              <div className="mt-2 flex items-center justify-between border-t border-border pt-2 text-[13px]">
                ...
                <span className={`tnum font-semibold ${impact.delta >= 0 ? 'text-destructive' : 'text-success'}`}>{impact.delta >= 0 ? '+' : ''}{currency(impact.delta, { compact: true })}</span>
              </div>
            </div>
            ...
            <p className="text-[11px] text-muted-foreground/75">Applying opens a new effective period from the chosen date; existing bills are untouched.</p>
          </CardBody>
        </Card>
      </ContentGrid>
      ...
      <Card>
        ...
        <CardBody className="flex flex-col gap-2">
          {peakWindows.map((pw) => (
            <div key={pw.id} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-muted/50 p-3">
              <div>
                <p className="text-[13px] font-medium text-foreground">{pw.label}</p>
                <p className="text-[12px] text-muted-foreground/75">Mon–Fri</p>
              </div>
              <span className="tnum rounded-lg bg-card px-3 py-1.5 text-[13px] font-medium text-foreground">{String(pw.startHour).padStart(2, '0')}:00 – {String(pw.endHour).padStart(2, '0')}:00</span>
            </div>
          ))}
          <p className="mt-1 text-[12px] text-muted-foreground/75">All hours outside these windows are billed at the off-peak rate. <span className="tnum">{num(kpis.metersTotal)}</span> meters are evaluated against these windows.</p>
        </CardBody>
      </Card>
    </Page>
  );
