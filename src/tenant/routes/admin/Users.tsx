...
import { Avatar, IconBox, StatusBadge } from '../../components/app/primitives';
...
export default function Users() {
  ...
  const columns: Column<(typeof users)[number]>[] = [
    { key: 'name', header: 'User', cell: (u) => <div className="flex items-center gap-2.5"><Avatar name={u.name} seed={u.avatarSeed} size={28} /><div><p className="font-medium text-foreground">{u.name}</p><p className="text-[11px] text-muted-foreground/75">{u.email}</p></div></div>, sortValue: (u) => u.name },
    ...
    { key: 'last', header: 'Last active', cell: (u) => <span className="text-muted-foreground/75">{u.lastActiveAt ? ago(u.lastActiveAt) : '—'}</span>, hideBelow: 'lg' },
  ];
      ...
      <ContentGrid align="start">
        <Card>
          ...
          <CardBody className="flex flex-col gap-2">
            {admins.map((a) => (
              <div key={a.id} className="flex items-center gap-3 rounded-xl border border-border bg-muted/50 p-3.5">
                <Avatar name={a.name} seed={a.avatarSeed} size={40} />
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[14px] font-medium text-foreground">{a.name}</p>
                  <p className="truncate text-[12px] text-muted-foreground/75">{a.title} · {a.org}</p>
                </div>
          ...
          <CardBody>
            <div className="mb-3 flex flex-col gap-2">
              {(Object.keys(ADMIN_ROLE) as (keyof typeof ADMIN_ROLE)[]).map((role) => (
                <div key={role} className="flex items-center gap-2 rounded-xl border border-border bg-muted p-3">
            ...
            <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.1em] text-muted-foreground/75">Planned roles</p>
            <div className="flex flex-wrap gap-1.5">
              {FUTURE_ROLES.map((r) => <span key={r} className="rounded-lg border border-border bg-muted px-2.5 py-1 text-[12px] text-muted-foreground">{r}</span>)}
            </div>
          </CardBody>
        </Card>
      </ContentGrid>
